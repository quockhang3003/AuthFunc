...
public class ProductRepository : BaseRepository<Product, int>, IProductRepository
    ...
    public async Task<PaginatedResponse<ProductDto>> SearchProductsAsync(ProductSearchRequest request)
    {
        ...
        { 
            whereBuilder.Append(" AND (p.name LIKE @Search OR p.description LIKE @Search OR CAST(p.tags AS NVARCHAR(MAX)) LIKE @Search)");
        ...
        {
            // true -> 1
            whereBuilder.Append(" AND p.is_active = 1");
        }
        else if (request.IsActive.HasValue)
        {
            whereBuilder.Append(" AND p.is_active = @IsActive");
        ...
        if (request.Tags != null && request.Tags.Length > 0)
        {
            ...
            {
                tagConditions.Add($"JSON_VALUE(p.tags, '$[{i}]') IN @Tags OR p.tags LIKE @TagSearch{i}");
                parameters.Add($"TagSearch{i}", $"%\"{request.Tags[i]}\"%");
            }

            var tagSearchConditions = string.Join(" OR ",
                request.Tags.Select((tag, index) =>
                {
                    parameters.Add($"Tag{index}", $"%{tag}%");
                    return $"p.tags LIKE @Tag{index}";
                }));

            whereBuilder.Append($" AND ({tagSearchConditions})");
        }
        ...
        var productDto = results.Select(MapToProductDto).ToList();

        return PaginatedResponse<ProductDto>.Create(productDto, totalCount, request.PageNumber, request.PageSize);
    }
