...
public class AuthService : IAuthService
{
    ...
    public async Task<ApiResponse<AuthenticationResponse.AuthResponse>> WindowsLoginAsync(
        AuthenticationRequest.WindowsLoginRequest request,
        ...
        string? userAgent = null)
    {
        try
        {
            ...
            var username = windowsIdentity.Split('\\').Last();

            // Validate with Active Directory
            var isValid = await _activeDirectoryService.ValidateUserAsync(username);
            if (!isValid)
            {
                _logger.LogWarning("Windows login failed - user not valid in AD: {WindowsIdentity}", windowsIdentity);
                return ApiResponse<AuthenticationResponse.AuthResponse>.ErrorResponse(
                    "User not found or disabled in Active Directory");
            }

            // Get user info from AD
            var adUserInfo = await _activeDirectoryService.GetUserInfoAsync(username);
            if (adUserInfo == null)
            {
                _logger.LogWarning("Windows login failed - cannot get AD info: {WindowsIdentity}", windowsIdentity);
                return ApiResponse<AuthenticationResponse.AuthResponse>.ErrorResponse(
                    "Cannot retrieve user information from Active Directory");
            }

            // Check if user exists in database
            var user = await _userRepository.GetByWindowsIdentityAsync(windowsIdentity);

            var enableAutoProvision = _configuration.GetValue<bool>("ActiveDirectory:EnableAutoProvision", true);
            ...
            if (user == null)
            {
                ...
                {
                    return ApiResponse<AuthenticationResponse.AuthResponse>.ErrorResponse(
                        "User not found in system. Auto-provisioning is disabled.");
                }

                // Auto-create user from AD info
                var permissions = await _activeDirectoryService.MapGroupsToPermissionsAsync(adUserInfo.Groups);
                ...
                _logger.LogInformation("Auto-provisioned Windows user from AD: {WindowsIdentity}, Groups: {Groups}",
                    windowsIdentity, string.Join(", ", adUserInfo.Groups));
            }
            else
            {
                // Sync with AD on login if enabled
                var syncOnLogin = _configuration.GetValue<bool>("ActiveDirectory:SyncOnLogin", true);

                if (syncOnLogin)
                {
                    // Update permissions from AD groups
                    var permissions = await _activeDirectoryService.MapGroupsToPermissionsAsync(adUserInfo.Groups);

                    if (user.Permissions != permissions || user.Email != adUserInfo.Email)
                    {
                        user.Permissions = permissions;
                        user.Email = adUserInfo.Email;
                        user.IsActive = adUserInfo.IsEnabled;
                        user.UpdatedAt = DateTime.UtcNow;

                        await _userRepository.UpdateAsync(user);

                        _logger.LogInformation("Synced user {WindowsIdentity} with AD. New permissions: {Permissions}",
                            windowsIdentity, permissions);
                    }
                }

                // Check if user is still active in AD
                if (!adUserInfo.IsEnabled)
                {
                    _logger.LogWarning("Windows login attempt for disabled AD account: {WindowsIdentity}", windowsIdentity);
                    return ApiResponse<AuthenticationResponse.AuthResponse>.ErrorResponse("Account is disabled in Active Directory");
                }
            }

            if (!user.IsActive)
            {
                _logger.LogWarning("Windows login attempt for inactive user: {WindowsIdentity}", windowsIdentity);
