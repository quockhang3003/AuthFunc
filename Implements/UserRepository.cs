...
public class UserRepository : BaseRepository<User, int>, IUserRepository
{
    ...
    public async Task<User?> GetByUsernameAsync(string username)
    {
        ...
        var sql = $"SELECT {GetSelectColumns()} FROM {_tableName} WHERE username = @Username AND is_active = 1";
    ...
    {
        using var connection = await _connectionFactory.CreateConnectionAsync();
        var sql = $"SELECT {GetSelectColumns()} FROM {_tableName} WHERE email = @Email AND is_active = 1";
    ...
    public async Task<User?> GetByWindowsIdentityAsync(string windowsIdentity)
    {
        using var connection = await _connectionFactory.CreateConnectionAsync();
        var sql =
            $"SELECT {GetSelectColumns()} FROM {_tableName} WHERE windows_identity = @WindowsIdentity AND is_active = 1";
    ...
    public async Task<PaginatedResponse<UserDto>> SearchUsersAsync(UserSearchRequest request)
    {
        ...
        {
            whereBuilder.Append(" AND (username COLLATE SQL_Latin1_General_CP1_CI_AS LIKE @Search OR email LIKE @Search)");
    ...
    public async Task<IEnumerable<User>> GetUsersByPermissionAsync(Permission.Permissions permission)
    {
        using var connection = await _connectionFactory.CreateConnectionAsync();
        var sql =
            $"SELECT {GetSelectColumns()} FROM users WHERE (permissions & @Permission) = @Permission AND is_active = 1";
